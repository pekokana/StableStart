...
for agent in agents:
    horses = []
    for _ in range(HORSES_PER_AGENT):
        horse = create_horse(agent["strategy"])

        # 評判が高いほど選ばれやすい
        trainer = random.choices(
            trainers,
            weights=[t.reputation for t in trainers]
        )[0]
...
# 年間実況生成
# ==============================
def generate_commentary(gen, distance, strategy_money, type_money, ai_money, trainers):

    dominant_strategy = max(strategy_money, key=strategy_money.get)
    dominant_type = max(type_money, key=type_money.get)

    top_trainer = max(trainers, key=lambda t: t.reputation)
    ...
    # 戦略覇権
    text += f"\n現在の覇権戦略は『{dominant_strategy}型』。"

    # 思想支配
    text += f"\n市場思想は『{dominant_type}型』が優勢。"

    # AI状況
    if ai_money > 2_000_000:
        text += "\nAIは資本を拡大中。"
    elif ai_money < 500_000:
        text += "\nAIは市場で苦戦している。"

    # トレーナー
    text += f"\n最強調教師は {top_trainer.name}（{top_trainer.skill}） 評判{round(top_trainer.reputation,2)}。"
...
for gen in range(GENERATIONS):
    ...
    for trainer in trainers:
        if trainer.aging():
            print(f"{trainer.name}（{trainer.skill}）が引退した。")
        else:
    ...
    top_trainer_rep = max([t.reputation for t in trainers])
    avg_trainer_rep = np.mean([t.reputation for t in trainers])
    ...
    history.append({
        ...
        "top_trainer_rep": top_trainer_rep,
        "avg_trainer_rep": avg_trainer_rep,
    ...
    if gen % 1 == 0:  # 毎世代出す（重いなら %10）
        ...
        ranking_lines = []
        ranked = sorted(trainers, key=lambda t: t.reputation, reverse=True)

        ranking_lines.append("🏆 調教師ランキング")
        for i, t in enumerate(ranked[:5], 1):
            ranking_lines.append(
                f"{i}位: {t.name} | skill:{t.skill} | personality:{t.personality} "
                f"| reputation:{round(t.reputation,2)} | horses:{len(t.horses)}/{t.max_horses}"
            )
        ...
        narrative_history.append(full_text)
...
filename = f"simulation_ver10_log_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
narrative_filename = f"simulation_ver10_history_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
print(f"{narrative_filename} に実況履歴を保存しました")
